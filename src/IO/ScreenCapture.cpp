...
std::shared_ptr<Texture> ScreenCapture::GetScreenFrameBuffer()
{
#ifdef __APPLE__
	CGRect r;
	r.origin.x = 0;
	...
	auto res = std::make_shared<Texture>(CGImageGetWidth(img), CGImageGetHeight(img), GL_RGB, GL_RGB, GL_UNSIGNED_BYTE);
	res->SetData(rawRGB);
	CFRelease(data);
